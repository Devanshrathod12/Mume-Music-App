    ...
    <Modal visible={visible} transparent animationType="none" onRequestClose={closeWithAnimation}>
      ...
      <Animated.View 
        style={[
            ...
            { 
                backgroundColor: theme.ModalBackground, // Dynamic BG
                transform: [{ translateY: Animated.add(slideAnim, translateY) }] 
            }
        ]}
      >
          {/* Header section with Pan Handler */}
          <View {...panResponder.panHandlers} style={[styles.dragHeader, { backgroundColor: theme.ModalBackground }]}>
               ...
               <View style={styles.artistHeader}>
                   <Image source={{ uri: getImageUrl(artist.image) }} style={[styles.headerImage, { backgroundColor: theme.LightGray }]} />
                   <View style={styles.headerTextContainer}>
                        <Text style={[styles.artistTitle, { color: theme.HeadingColor }]} numberOfLines={1}>{artist.name}</Text>
                        <Text style={[styles.artistSub, { color: theme.SecondaryText }]}>Artist  |  {songCount} Songs</Text> 
                   </View>
               </View>
              ...
              {menuItems.map((item, index) => (
                  ...
                  >
                      <Ionicons 
                        ...
                        color={theme.HeadingColor} // Dynamic Icon Color
...
const styles = StyleSheet.create({
  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)' }, // Darkened slightly for better focus
  modalContainer: { 
    height: '56%', 
    width: '100%', 
