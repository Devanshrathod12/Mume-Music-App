...
const AlbamSongList = () => {
  ...
  const handleShufflePlay = async () => {
    ...
    // 1. Pehle normal list taiyar karo (jaise play all mein karte ho)
    const tracksToAdd = songs.map(s => ({
        id: s.id,
        url: getAudioUrl(s),
        title: s.name,
    ...
    try {
        // 2. CORE LOGIC: Taiyar ki gayi list ko random mix kar do
        const shuffledTracks = shuffleArray(tracksToAdd);

        // 3. Player ko reset karke shuffled list load karo
        await TrackPlayer.reset();
        await TrackPlayer.add(shuffledTracks);
        await TrackPlayer.play();

        // 4. Storage update karo (taki mini player aur queue sync rahe)
        saveQueueToStorage(shuffledTracks, 0);

        // 5. Player screen par bhej do
        navigation.navigate('MusicPlayer');
    } catch (e) {
        console.log("Shuffle Play Error:", e);
    }
};
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.WhiteBackground }]}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
             ...
             <TouchableOpacity>
                <Ionicons name="ellipsis-horizontal-circle-outline" size={24} color={theme.Black} />
             </TouchableOpacity>
        </View>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
          ...
          <View style={styles.heroSection}>
              ...
              <Text style={[styles.albumTitle, { color: theme.HeadingColor }]} numberOfLines={2}>
                 {albumData?.name || "Unknown Album"}
              </Text>
              
              <View style={styles.metaContainer}>
                  <Text style={[styles.metaText, { color: theme.SecondaryText }]}>Album</Text>
                  <Text style={[styles.metaDivider, { color: theme.Separator }]}>|</Text>
                  <Text style={[styles.metaText, { color: theme.SecondaryText }]}>{loading ? '...' : songs.length} Songs</Text>
                  <Text style={[styles.metaDivider, { color: theme.Separator }]}>|</Text>
                  <Text style={[styles.metaText, { color: theme.SecondaryText }]}>{albumData?.year || "2023"}</Text>
              </View>
              ...
              <View style={styles.buttonRow}>
                  ...
                  <TouchableOpacity 
                    style={[styles.actionButton, { backgroundColor: theme.CardBackground, borderWidth: 1, borderColor: theme.Separator }]} 
                    onPress={handlePlayAll}
                  >
                      <Ionicons name="play-circle" size={22} color={theme.Primary} style={{marginRight: 8}} />
                      <Text style={[styles.btnPlayText, { color: theme.Primary }]}>Play</Text>
                  </TouchableOpacity>
              </View>
          </View>

          <View style={styles.sectionHeader}>
              <Text style={[styles.sectionTitle, { color: theme.HeadingColor }]}>Songs</Text>
              <Text style={[styles.seesectionTitle, { color: theme.Primary }]}>See All</Text>
          </View>
                ...
                {songs.map((song) => {
                     ...
                     return (
                        <View key={song.id} style={[
                            styles.songRow, 
                            isCurrent && { backgroundColor: theme.CardBackground, borderRadius: 12 }
                        ]}>
                            <TouchableOpacity onPress={() => handlePlayPause(song)}>
                                <Image 
                                    source={{ uri: getImageUrl(song.image) }} 
                                    style={[styles.songImage, { backgroundColor: theme.LightGray }]} 
                                />
                            </TouchableOpacity>
                            ...
                            <TouchableOpacity style={styles.songInfo} onPress={() => handlePlayPause(song)}>
                                <Text style={[
                                    styles.songTitle, 
                                    { color: isCurrent ? theme.Primary : theme.HeadingColor }
                                ]} numberOfLines={1}>
                                    {song.name}
                                </Text>
                            ...
                            <View style={styles.songActions}>
                                <TouchableOpacity onPress={() => handlePlayPause(song)}>
                                    <Ionicons name={iconName} size={30} color={theme.Primary} />
                                </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    albumTitle: { fontSize: textScale(22), fontWeight: 'bold', textAlign: 'center', marginBottom: verticalScale(8), marginTop: verticalScale(10) },
    metaContainer: { flexDirection: 'row', alignItems: 'center', marginBottom: verticalScale(25) },
    metaText: { fontSize: textScale(12), fontWeight: '500' },
    metaDivider: { marginHorizontal: scale(8) },
    buttonRow: { flexDirection: 'row', width: '100%', justifyContent: 'space-between', marginBottom: verticalScale(30) },
    actionButton: { flex: 0.48, height: verticalScale(50), borderRadius: 30, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' },
    btnShuffleText: { color: '#FFF', fontSize: textScale(16), fontWeight: '600' },
    btnPlayText: { fontSize: textScale(16), fontWeight: '600' },
    sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: scale(20), marginBottom: verticalScale(15) },
    sectionTitle: { fontSize: textScale(18), fontWeight: 'bold' },
    seesectionTitle: { fontSize: textScale(14), fontWeight: 'bold' },
    songsList: { paddingHorizontal: scale(20) },
    songRow: { flexDirection: 'row', alignItems: 'center', marginBottom: verticalScale(8), padding: scale(8) },
    songImage: { width: moderateScale(50), height: moderateScale(50), borderRadius: 10, marginRight: scale(15) },
    songInfo: { flex: 1 },
    songTitle: { fontSize: textScale(15), fontWeight: 'bold', marginBottom: 3 },
    songArtist: { fontSize: textScale(12), fontWeight: '500' },
    songActions: { flexDirection: 'row', alignItems: 'center' },
});